...
export function Topbar({ toggleSidebar }: TopbarProps) {
    ...
    const handleScan = async (e: React.FormEvent) => {
        e.preventDefault();
        const enteredUrl = scanUrl.trim();
        if (!enteredUrl) return;

        setLoading(true);
        try {
            const normalizedUrl = /^https?:\/\//i.test(enteredUrl) ? enteredUrl : `https://${enteredUrl}`;
            const res = await api.post('/scans', { url: normalizedUrl });
            addToast('Scan completed successfully', 'success');
            navigate(`/reports/${res.data._id}`);
        } catch (err) {
            console.error(err);
            addToast(isAxiosError(err) ? err.response?.data?.msg || 'Scan failed. Please try again.' : 'Scan failed. Please try again.', 'error');
        } finally {
            setLoading(false);
            setScanUrl('');
        }
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-800 bg-slate-900/50 px-4 md:px-6 backdrop-blur">
            <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1 flex items-center gap-4">
                    <form onSubmit={handleScan} className="hidden md:flex relative group max-w-xl w-full items-center gap-2">
                        <label htmlFor="topbar-scan-url" className="sr-only">Scan a URL</label>
                        <div className="relative flex-1">
                            ...
                            <Input
                                ...
                                className="pl-10 pr-3 bg-slate-900 border-slate-800 focus:border-primary-500/50 transition-all placeholder:text-slate-500"
                                value={scanUrl}
                        ...
                        <Button
                            ...
                            aria-label="Scan URL"
                        >
                            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><ScanSearch className="mr-1.5 h-4 w-4" />Scan</>}
                        </Button>
                    </form>
                </div>
            </div>
